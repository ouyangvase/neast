...
class CouponCategoryService
{
    ...
    public function list(RequestInterface $request): array
    {
        ...
        $query = CouponCategoryModel::query();
        ...
        $items = $query->orderBy('id', 'desc')
            ->forPage($page, $limit)
            ->get()
            ->map(fn (CouponCategoryModel $category) => $category->toArray())
            ->all();
    ...
    public function create(array $params): array
    {
        ...
        $category = new CouponCategoryModel();
        $category->name = $name;
        $category->save();
    ...
    {
        return CouponCategoryModel::query()
            ->orderBy('id', 'asc')
            ->get(['id', 'name'])
            ->toArray();
    }
    ...
    private function assertNameUnique(string $name, ?int $excludeId = null): void
    {
        ...
        $query = CouponCategoryModel::withTrashed()->where('name', $name);
        if ($excludeId !== null) {
    ...
    private function findOrFail(int $id): CouponCategoryModel
    {
        $category = CouponCategoryModel::query()->find($id);
        if (! $category) {
