...
/// 选取物业照片。
Future<({File file, String name})?> pickPropertyPhoto(
  ...
  final name = picked.name.isNotEmpty ? picked.name : 'property_photo.jpg';
...
/// 选取物业文档。
Future<({File file, String name})?> pickPropertyDocument(
  BuildContext context,
) async {
          ...
          children: [
            ListTile(
              ...
              onTap: () => Navigator.of(sheetContext)
                  .pop(_DocumentSource.gallery),
            ),
            ListTile(
              leading: Icon(Icons.folder_outlined, color: brandBlue),
              title: const Text('Browse Files'),
              onTap: () =>
                  Navigator.of(sheetContext).pop(_DocumentSource.files),
            ),
  ...
  switch (source) {
    case _DocumentSource.gallery:
      ...
      final bytes = await picked.readAsBytes();
      final name =
          picked.name.isNotEmpty ? picked.name : 'property_document.jpg';
      final tempFile = File('${Directory.systemTemp.path}/$name');
      await tempFile.writeAsBytes(bytes, flush: true);
      return (file: tempFile, name: name);
...
String truncatePropertyFileName(String name, {int maxLength = 18}) {
