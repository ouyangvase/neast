...
import 'package:neast/features/wallet/widgets/wallet_topup_month_picker_sheet.dart';
...
class _WalletScreenState extends ConsumerState<WalletScreen> {
  ...
  Future<void> _onMonthTap() async {
    ...
    final selected = await showWalletTopupMonthPicker(
      context,
      selected: listNotifier.selectedMonth,
      options: rollingWalletTopupMonths(),
    );
    if (selected == null || !mounted) return;
    await listNotifier.setMonth(selected);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final balanceDisplay = balanceAsync.maybeWhen(
      data: formatWalletBalanceDisplay,
      orElse: () => '0',
    );
