...
class PushNotificationService {
  ...
  /// iOS 必须先拿到 APNS Token，FCM Token 才能生成。
  ...
  Future<void> _saveTokenToServer(String token) async {
    try {
      ...
      if (accessToken == null || accessToken.isEmpty) {
        logger.d('用户未登录，暂不保存 FCM Token');
        return;
      }

      final platform = Platform.isIOS ? 'ios' : 'android';

      await _dioClient.post('/app/push/add-fcm-token', data: {
        'token': token,
        'platform': platform,
      });
  ...
  void _refreshNotificationList() {
    try {
      ...
      _ref.read(notificationUnreadProvider.notifier).refresh();
      logger.d('已触发消息列表与未读状态刷新');
    } catch (e) {
      logger.e('刷新消息数据失败: $e');
    }
  }
  ...
  void _navigateByType(String? type, int? _) {
    final router = _ref.read(appRouterProvider);

    switch (type) {
      case 'PointAdd':
        router.push(AppRoutes.points);
      case 'Coupon':
        router.push(AppRoutes.myVouchers);
      case 'Test':
      default:
  ...
  Future<void> deleteToken() async {
    try {
      ...
      if (token != null) {
        try {
          await _dioClient.post('/app/push/delete-fcm-token', data: {
            'token': token,
          });
        } catch (e) {
