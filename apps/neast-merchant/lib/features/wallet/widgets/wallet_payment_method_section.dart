...
class _WalletPaymentMethodSectionState
  ...
  List<PaymentMethodItem> _buildMethods(Map<String, double> fees) {
    final baseMethods = widget.showWalletOption
        ? WalletConfig.allPaymentMethods
        : WalletConfig.topupPaymentMethods;
    ...
    return baseMethods.map((method) {
      ...
      final feePercent = fees[method.id] ?? 0;
      return PaymentMethodItem(
        id: method.id,
        label: method.label,
        trailing: formatProcessingFeeLabel(feePercent),
        totalAmountLabel: formatTotalAmountWithFee(widget.amount, feePercent),
      );
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final methods = _buildMethods(fees);
